		...
		function showTip() {
			...
			if (chartType === "pie" || chartType === "radar") {
				...
				chart.dispatchAction({
					type: "downplay",
					seriesIndex: options.loopSeries ? (seriesIndex === 0 ? seriesLen - loopStep : seriesIndex - loopStep) : seriesIndex,
					dataIndex: dataIndex === 0 ? dataLen - loopStep : dataIndex - loopStep,
				});
			...
			dataIndex = (dataIndex + loopStep) % dataLen;
			if (options.loopSeries && dataIndex === 0 && !first) {
				// 数据索引归0表示当前系列数据已经循环完
				seriesIndex = (seriesIndex + loopStep) % seriesLen;
			}
